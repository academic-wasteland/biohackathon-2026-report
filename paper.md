...
The Global Alliance for Genomics and Health (GA4GH) Task Execution Service (TES) provides a common interface for batch tasks across computing environments, including execution near distributed data [@TES]. Its Workflow Execution Service (WES) exposes workflow submission and management [@WES]. These are natural execution backends for a research node. Wasteland's contribution lies in connecting the surrounding discovery, delegation and admission decisions to such local execution capabilities; a GA4GH adapter would require its own implementation and tests.
...
Workflow Run RO-Crate packages computational inputs, outputs, software and execution provenance, with implementations across several workflow systems [@WorkflowRunCrate]. It provides a suitable foundation for the proposed packaging of a complete Wasteland interaction. At present, the prototype links messages and produces individual validation and interpretation reports. Together, these predecessors locate the contribution of this hackathon in an executable integration of established ideas: discovering a scientific capability, checking authority to use it, carrying out the allowed operation and preserving evidence of the exchange.
...
| GA4GH TES/WES and Passports | Portable execution and federated access information | Custodian and executor decisions joined in a working application; standards adapters remain prospective. |
| WfExS, RO-Crate and FAIR Digital Objects | Reproducible execution, provenance packaging and discovery | Linked message and decision traces; complete interoperable run packages remain prospective. |
