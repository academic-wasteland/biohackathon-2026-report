...
abstract: |
  Using another laboratory's scientific services requires researchers to discover capabilities, translate requests, coordinate execution and respect restrictions on data use. Shared conventions could let agents perform this coordination while each laboratory retains control of its resources. During DBCLS BioHackathon 2026, we developed Academic Wasteland, a prototype research commons organised into independently configurable nodes called towns. The implementation combines authenticated message exchange, provider-owned semantic service descriptions, receiver-controlled authorisation, and versioned conformance profiles. FAIRhaven provides discovery and bounded service checks, while Camelot hosts reviewed credential issuance. Sysifoss adds a GA4GH TES compute broker and RO-Crate message packaging. Demonstrations cover permission-gated cohort comparison and a visitor's synthetic data processed on remote compute. A synthetic rare-disease workflow connects phenotype-based gene ranking at one town with local variant filtering at another and returns a report based on pinned public expert evidence. In the documented demonstration, FBN1 ranked third among 1,529 mouse gene profiles. Four of six synthetic variant calls passed filtering, and a specified combination of phenotype rank and variant score placed the selected FBN1 allele first. The workflow exposes its inter-town messages and keeps the complete variant file within the local analysis step. Separate executable certification scenarios exercise scoped trust, holder binding, delegation and revocation. These results establish a working integration of discovery, local policy and bounded scientific services. The present deployment uses a shared relay and co-hosted demonstration towns; evaluation across independent implementations, stronger host isolation and complete reproducibility packaging remain subsequent milestones.
---
...
A town at Maastricht contributes a different kind of capability: statistical summaries over de-identified intensive-care records rather than a genomic analysis. It answers a fixed set of cohort queries over MIMIC-IV and releases no record under any operation. Six conditions must hold before an answer is computed: a credential signed by an issuer the town accredits, a fresh signed status for that credential, proof that the requester holds the key it names, an undertaking signed and given to this town, a query drawn from an allowlist, and a check against what the same requester has already been told. The captured run reported in-hospital mortality by admission type over 94,458 intensive-care stays, from 13.4 per cent for emergency admissions to 1.9 per cent for same-day surgical ones. A second query, differing from the first by a single stay, was refused before any answer was released, because subtracting the two would have described that patient [@ZerzuraDemo].

This case demonstrates that the quantity released, and not only the requester's authority, can determine admission. Suppression is a property of the dataset here: an openly licensed extract is served over the relay without it, while the credentialed database receives it and is queried locally only. Releasing that database is a separate institutional decision which this work does not make.

## An independently implemented client and town

A separately developed client and town, implemented at Maastricht University, tested the credential and status profile against Camelot. The client was written from the protocol documentation alone, using only the Python standard library rather than the reference dependency, and the town ran on separate hardware under its own access policy [@Zerzura]. Camelot issued a holder-bound credential to a key generated there after human review; the independent verifier accepted it, and refused a changed query, a different requesting town, a replayed challenge and conflicting holder-key aliases. The registrar's signed status statements verified against issuer keys held in the receiving town's own trust store, which selects its own roots: Camelot's deliberately unaccredited test issuer was refused until its exact identifier was named as a root, and the refusal was confirmed to return once that was removed. The exercise identified two defects. An issuer allowlist expressed as a URL prefix admitted every issuer the registrar publishes, including that test issuer; and a client helper discarded the operation when a message body was supplied, producing a misleading reply. Both were corrected and regression-tested, and the independent verifier is now pinned in the provider's continuous integration [@ZerzuraInterop]. The relay remained shared infrastructure, so this establishes implementation interoperability and separate administration rather than independent institutional federation.
